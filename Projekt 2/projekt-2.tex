...
W przypadku zadania niepodzielnego model przedstawia się następująco: \\ \\
Zmienne: \\
\(x_{ij}\) - zmienna określająca obsłużenie zadania \(i\) na procesorze \(j\). \\ \\
\(p_j\) - czas przetwarzania zadania j \\ \\
Funkcja celu: \\
\begin{align*}
    min \text{ } C_{max}
...
Przy ograniczeniach: \\
\begin{align*}
    & \sum^n_{i=1}\sum^m_{j=1}x_{ij}p_{ij} \leq C_{max} \\
    & \sum^n_{i=1}\sum^m_{j=1}x_{ij} = 1 \\
    & x \in  {0,1}
\end{align*}
gdzie: \\
\begin{align*}
...
\\ \\
Proponowaną regułą przydziału zadań minimalizującąn czas wykonywania wszystkich
zadań (\(C_{nax}\) jest szeregowanie listowe przy użyciu zasady LPT.

\subsection{Szeregowanie zadań na niejednorodowych procesorach równoległych}
\subsubsection{Zadania podzielne}
Model dla tego zadania został zdefiniowany w następujący sposób: \\
\(N\) - zadania \\
\(P\) - procesory \\
\\
Zmienne: \\
\(p_{ij}\) - czas przetwarzania zadania \(j\) na procesorze \(i\) \\
\\
Funkcja celu: \\
\begin{align*}
    \text{min } C_{max}
